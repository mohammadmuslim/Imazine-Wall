...
<body>
    ...
    <h4 style="color: black; padding-bottom: 0">
	  <strong>Company  
       ...
       Date To <span style="color: red">{{ date('d-m-Y', strtotime($end_date)) }} )</span> Date Total Amount 
	</strong>
	</h4>
<table border="1" width="100%" style="text-align: center;">
	...
	<tbody>
		@foreach($companyTotals as $key => $row)
		<tr>
			<td>{{ $key+1 }}</td>
			<td>{{ date('d-m-Y', strtotime($row->date)) }}</td>
			<td>{{ $row->today_sales }}  TK</td>
		</tr>
		@endforeach
		<tr>
			<td style="text-align: right; color:green;" colspan="2">Total Amount:-</td>
			<td style="color:green;">{{ $companyTotalAmount }}  TK</td>
		</tr>
	</tbody>
</table>
@php 
